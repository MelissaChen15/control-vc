"""
Main Traning Loop for Hifi-GAN Vocoder
...
def train(rank, local_rank, a, h):
    ...
    trainset = CodeDataset(training_filelist, h.segment_size, h.code_hop_size, h.n_fft, h.num_mels, h.hop_size,
                           ...
                           random_sample=h.get('random_sample', False),
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            x_needs_grad = {}
            for k, v in x.items():
                if torch.is_tensor(v): x_needs_grad[k] = torch.autograd.Variable(v.to(device, non_blocking=False))
            x = x_needs_grad
...
def main():
    ...
    parser.add_argument('--config', default='configs/hifigan.json')
