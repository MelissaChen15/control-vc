...
def train(rank, a, h):
    ...
    cp_g = None # generator check point
    ...
    optim_g = torch.optim.AdamW(generator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2]) # generator optimizer

    if state_dict_g is not None:
        optim_g.load_state_dict(state_dict_g['optim_g'])

    scheduler_g = torch.optim.lr_scheduler.ExponentialLR(optim_g, gamma=h.lr_decay, last_epoch=last_epoch) # generator scheduler
    ...
    if rank == 0:
        validset = F0Dataset(validation_filelist, h.segment_size, h.sampling_rate, False, n_cache_reuse=0,
                             device=device, f0_stats=h.get('f0_stats', None),
                             f0_normalize=h.get('f0_normalize', False), f0_feats=h.get('f0_feats', False),
                             f0_median=h.get('f0_median', False), f0_interp=h.get('f0_interp', False),
                             vqvae=h.get('code_vq_params', False))
        ...
        sw = SummaryWriter(os.path.join(a.checkpoint_path, 'logs')) # write tensorboard summary
...
def main():
    ...
    parser.add_argument('--checkpoint_path', default='checkpoints/debug')
    parser.add_argument('--config', default='configs/f0_vqvae.json')
