...
class CodeGenerator(Generator):
    '''
    Generating waveform from codes, not for generating code
    '''
    def __init__(self, h):
        super().__init__(h)
        ...
        self.rate = h.get('rate', False)
        self.boundary = h.get('boundary', False)
    ...
    def forward(self, **kwargs):
        ...
        if self.f0:
            x_len = x.shape[-1]
            f0_len = kwargs['f0'].shape[-1]

            if x_len < f0_len: # len(code) < len(f0), upsample code to match
                n_reminder = f0_len % x_len
                x = self._upsample(x, f0_len-n_reminder)
                if n_reminder > 0:
                    x = torch.cat((x,x[:, :, 0:n_reminder]), 2)

                x = self._upsample(x, kwargs['f0'].shape[-1])
            else:
                # truncate f0 
                n_reminder = x_len % f0_len
                kwargs['f0'] = self._upsample(kwargs['f0'], x_len-n_reminder)
                if n_reminder > 0:
                    kwargs['f0'] = torch.cat((kwargs['f0'], kwargs['f0'][:, :, 0:n_reminder]), 2)

            x = torch.cat([x, kwargs['f0']], dim=1)
            

        if self.rate:
            kwargs['rate'] = kwargs['rate'].unsqueeze(1)
            x = torch.cat([x, kwargs['rate']], dim=1)


        spk_embed = kwargs['spk_embed']
        spk_embed = self._upsample(spk_embed, x.shape[-1])
        ...
        for k, feat in kwargs.items():
            if k in ['spkr_name', 'spk_embed', 'code', 'f0', 'boundary', 'rate']:
                continue
